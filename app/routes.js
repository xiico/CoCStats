...
module.exports = function (app, passport) {
  ...
  app.get('/:lang?/player/:id', /*isLoggedIn,*/ function (req, res) {
    ...
    db.searchPlayers('Tag', req.params.id.toUpperCase(), null, function (err, player) {
      ...
      if (err) RenderPage('error', req, res, [], {}, err.reason, {status:"Player Error",stack:"Error"});
      else
  ...
  app.get('/:lang?/players/rank/:location?', /*isLoggedIn,*/ function (req, res) {
    ...
    db.getClanPlayerRank({ latest: false, location: req.params.location }, function (err, clanPlayers) {
      if(err || !clanPlayers || !clanPlayers.items.length) return RenderPage('error', req, res, [], {}, "Not Found", {status:"Players Rank Error",stack:"Page not found"});
